...
public class Measure extends Parameter{
    ...
    protected void saveComplete() {
        //Every lazy Measurement()/ConvertTo()/Measure() accessor across the model layer serves
        //repeat lookups for this id from ReferenceCache - drop the stale snapshot now that the
        //record has changed.
        ReferenceCache.invalidate("Measure", poModel.getMeasureId());
    }
